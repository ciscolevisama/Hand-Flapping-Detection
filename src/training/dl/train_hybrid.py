# -*- coding: utf-8 -*-
"""
Hybrid CNN-LSTM training for 8-class hand-flapping classification.

与 train_cnn.py / train_lstm.py 完全对齐的管线：
- 24通道特征(原8 + Δ + Δ²)、NaN-safe 归一化、分组分层CV、置信度加权
- 类不平衡：sampler / class_weight / both；支持 Focal Loss
- Fine-tune: 冻结 cnn / lstm / early2(仅冻结LSTM第0层±反向) / encoder(特征CNN+LSTM)

示例（初训）：
python src\\training\\dl\\train_hybrid.py ^
  --mode clean_only --seq_len 64 --epochs 30 --lr 5e-4 --cv 5 ^
  ...
  --cnn_channels 32,64,128 --kernel_size 5 --pool 2 ^
  --outdir runs_dl\\hybrid_len64_clean

示例（微调）：
python src\\training\\dl\\train_hybrid.py ^
  --mode clean_only --seq_len 64 --epochs 10 --lr 2e-4 --cv 5 ^
  --balancing both --sampler_alpha 0.7 --cw_beta 0.5 --cw_clip 3 ^
  --loss focal --gamma 2.0 --max_nan_ratio 0.6 ^
  --dropout 0.3 --hidden_size 128 --num_layers 1 ^
  --cnn_channels 32,64,128 --kernel_size 5 --pool 2 ^
  --freeze cnn --resume_from "runs_dl\\hybrid_len64_clean\\best_fold{fold}.pt" ^
  --outdir runs_dl\\hybrid_len64_clean_finetune
"""
import os, json, argparse, warnings, random, math
...
def compute_norm_stats_24(df_sub: pd.DataFrame, processed_dir: str, seq_len: int):
    ...
    counters = {"norm_total_vals": int(M.size), "norm_nan_vals": int(np.isnan(M).sum()),
...
class HybridCNNLSTM(nn.Module):
    """
    前端CNN抽局部时序+后端BiLSTM抽长程依赖，时间维做平均池化接分类头。
    """
    def __init__(self, in_ch: int = 24, n_classes: int = 8,
                 cnn_channels: Tuple[int, ...] = (32, 64, 128),
                 kernel_size: int = 5, pool: int = 2,
                 hidden_size: int = 128, num_layers: int = 1,
                 bidirectional: bool = True, dropout: float = 0.3):
        super().__init__()
        ks = kernel_size
        chs = list(cnn_channels)
        blocks = []
        cprev = in_ch
        for c in chs:
            blocks += [
                nn.Conv1d(cprev, c, ks, padding=ks//2, bias=False),
                nn.BatchNorm1d(c),
    ...
    def forward(self, x):               # x: (B, C, L)
        f = self.cnn(x)                 # (B, C', L')
        xseq = f.transpose(1, 2)        # (B, L', C')
        out, _ = self.lstm(xseq)        # (B, L', H*dir)
        h = out.mean(dim=1)             # time avg-pool
        return self.head(h)

# 供 ensemble 脚本导入的别名（保持一致性）
Hybrid1D = HybridCNNLSTM
CNNLSTM1D = HybridCNNLSTM
...
def apply_freeze(model: nn.Module, mode: str) -> int:
    ...
    if mode == "none":
        ...
        for p in model.cnn.parameters(): frz(p)
    elif mode == "lstm":
        for p in model.lstm.parameters(): frz(p)
    elif mode == "early2":
        for name, p in model.lstm.named_parameters():
            if "l0" in name: frz(p)  # 覆盖 l0 和 l0_reverse
    elif mode == "encoder":
        for p in model.cnn.parameters(): frz(p)
        for p in model.lstm.parameters(): frz(p)
    elif mode == "all":
        for p in model.parameters(): frz(p)
    else:
...
def main():
    ...
    # model
    ap.add_argument("--cnn_channels", type=str, default="32,64,128")
    ...
    # fine-tune
    ap.add_argument("--freeze", choices=["none","cnn","lstm","early2","encoder","all"], default="none")
    ...
    # 保存 arch 元信息（供将来需要时参考；ensemble 使用默认构造也可对上）
    chs = tuple(int(x) for x in args.cnn_channels.split(",") if x.strip())
    save_json({
        "arch": "hybrid",
        "in_ch": 24,
        "cnn_channels": chs,
    ...
    for fold, (tr_idx, va_idx) in enumerate(stratified_group_kfold(df, label2id, args.cv, args.seed), 1):
        ...
        # model / opt
        model = HybridCNNLSTM(
            in_ch=24, n_classes=len(classes),
            cnn_channels=chs, kernel_size=args.kernel_size, pool=args.pool,
    ...
    # overall
    pd.DataFrame(fold_rows).to_csv(os.path.join(args.outdir, "metrics_hybrid.csv"), index=False)
