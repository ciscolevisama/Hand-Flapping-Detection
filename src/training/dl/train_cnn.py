# -*- coding: utf-8 -*-
"""
Robust 1D-CNN training for 8-class hand-flapping form classification.

What’s added in this version:
- Dropout configurable (default 0.3) in all conv blocks.
- Fine-tune support: load checkpoint (--resume_from) and freeze early layers (--freeze).
  * --freeze choices: none | conv1 | early2 | backbone
  * You can pass a path template for --resume_from, e.g. ".../best_fold{fold}.pt"
- Everything else (NaN-safe preprocessing / CV / imbalance knobs / focal loss) preserved.

Typical fine-tune command (freeze first two conv blocks):
python src/training/dl/train_cnn.py ^
  --mode clean_only ^
  --seq_len 64 --epochs 10 --lr 2e-4 --cv 5 ^
  --balancing both --sampler_alpha 0.7 --cw_beta 0.5 --cw_clip 3 ^
  --loss focal --gamma 2.0 --max_nan_ratio 0.6 ^
  --dropout 0.3 --freeze early2 ^
  --resume_from "runs_dl\\cnn_len64_both_focal_nan06_new\\best_fold{fold}.pt" ^
  --outdir runs_dl\\cnn_len64_finetuned_early2
"""
...
# canonical label order (will be intersected with labels present in CSV)
TAXONOMY = [
    "no_flap",
...
# ------------------------ robust grouped stratification ------------------------
def stratified_group_kfold(df: pd.DataFrame, label2id: Dict[str,int], cv:int, seed:int, max_tries:int=50):
    """
    Try StratifiedGroupKFold if available; otherwise construct folds by greedily balancing
    per-fold label histograms over groups. Ensures: every TRAIN fold covers all labels.
    """
    ...
    if cv == 1:
        ...
        for _ in range(max_tries):
            ...
            if ok_train_has_all(tr_idx):
                yield tr_idx, va_idx; return
        ...
        if HAS_SGK:
            ...
            return
        # Greedy assignment if SGK missing
        rng = np.random.default_rng(seed)
        for _ in range(max_tries):
            vids = uniq_groups.copy(); rng.shuffle(vids)
            # label hist per group
            g2hist = {}
            for g in vids:
            ...
            for g in vids:
                # put group to fold that minimises label-imbalance L2
                costs = []
                for k in range(cv):
...
def make_features(x: np.ndarray, seq_len: int) -> np.ndarray:
    """
    x: (T, D=8) possibly with NaNs.
    Resample to seq_len, then stack [x, Δx, Δ²x] -> (L, 24)
    """
    ...
    if T != seq_len:
        ...
        # nan-aware interpolation per dim
        for d in range(D):
    ...
    feats = np.concatenate([xr[:, :8], delta[:, :8], delta2[:, :8]], axis=1)  # (L,24) use first 8 dims
    return feats.astype(np.float32)

def _nanfix_timewise(x24: np.ndarray) -> np.ndarray:
    """Per-channel linear interpolation over time; NaN-only channels -> 0."""
    L, C = x24.shape
    idx = np.arange(L)
    for c in range(C):
        col = x24[:, c]
...
def compute_norm_stats_24(df_sub: pd.DataFrame, processed_dir: str, seq_len: int) -> Tuple[np.ndarray, np.ndarray, Dict[str,int]]:
    """
    Returns: mean24 (1,24), std24 (1,24), and a small dict with nan counters for logging.
    """
    ...
    for _, r in df_sub.iterrows():
        ...
        x = vid[s:e, :8]  # only first 8 dims used
        rows.append(make_features(x, seq_len))
    M = np.concatenate(rows, axis=0)  # (N*L, 24)
...
class FlapDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        x = vid_arr[s:e, :8]                                          # (T,8)
        x24 = make_features(x, self.seq_len)                          # (L,24)
        x24 = _nanfix_timewise(x24)                                   # fill NaNs
        x24 = (x24 - self.mean24) / self.std24                        # normalise
        x24 = np.nan_to_num(x24, nan=0.0, posinf=0.0, neginf=0.0)     # final guard
...
# ------------------------ model & loss ------------------------
def _conv_block(in_ch, out_ch, k, p, dropout_p):
    layers = [
...
class CNN1D(nn.Module):
    def __init__(self, in_ch: int, n_classes: int, dropout: float = 0.3):
        super().__init__()
        self.block1 = _conv_block(in_ch,   64, k=5, p=2, dropout_p=dropout)
        self.block2 = _conv_block(64,     128, k=5, p=2, dropout_p=dropout)
        self.block3 = _conv_block(128,    128, k=3, p=1, dropout_p=dropout)
...
def freeze_layers(model: CNN1D, mode: str):
    """
    mode:
      - 'none'    : train all
      - 'conv1'   : freeze block1
      - 'early2'  : freeze block1 + block2
      - 'backbone': freeze block1 + block2 + block3 (only train FC)
    """
    to_freeze = []
    if mode == "conv1":
        to_freeze = [model.block1]
    elif mode == "early2":
        to_freeze = [model.block1, model.block2]
    elif mode == "backbone":
        to_freeze = [model.block1, model.block2, model.block3]
    elif mode == "none":
        to_freeze = []
    else:
        raise ValueError(f"--freeze expects one of none/conv1/early2/backbone, got {mode}")

    n_params = 0
    for mod in to_freeze:
...
def main():
    ...
    ap.add_argument("--dropout", type=float, default=0.3, help="dropout prob in conv blocks (default 0.3)")
    # fine-tune / resume
    ap.add_argument("--resume_from", type=str, default=None,
                    help="path to checkpoint (.pt). You can use '{fold}' placeholder for CV (e.g., '.../best_fold{fold}.pt').")
    ap.add_argument("--freeze", choices=["none","conv1","early2","backbone"], default="none",
                    help="freeze early layers when training (for fine-tune).")
    ap.add_argument("--patience", type=int, default=10)
    # imbalance
    ap.add_argument("--balancing", choices=["none","sampler","class_weight","both"], default="none")
    ap.add_argument("--sampler_alpha", type=float, default=0.5, help="1=1/freq, 0=uniform")
    ap.add_argument("--cw_beta", type=float, default=0.5, help="1=1/freq, 0=none (median^beta)")
    ...
    # data hygiene
    ap.add_argument("--max_nan_ratio", type=float, default=0.4,
                    help="drop window if NaN ratio over first 8 dims exceeds this (0~1)")
    args = ap.parse_args()
    ...
    for fold, (tr_idx, va_idx) in enumerate(stratified_group_kfold(df, label2id, args.cv, args.seed), 1):
        ...
        # ---------- hygiene: drop high-NaN windows (per first 8 dims) ----------
        drop_tr, keep_tr = 0, []
        cache = {}
        ...
        if drop_tr:
            print(f"[Fold {fold}] Dropped {drop_tr} train windows for NaN ratio > {args.max_nan_ratio}")
        df_tr = df_tr.iloc[keep_tr].reset_index(drop=True)

        # ---------- compute normalisation stats on TRAIN only (NaN-safe) ----------
        ...
        # ---------- model / opt ----------
        model = CNN1D(in_ch=24, n_classes=len(classes), dropout=args.dropout).to(device)

        # resume (for fine-tune)
        resume_path: Optional[str] = None
        if args.resume_from:
            resume_path = args.resume_from.format(fold=fold) if "{fold}" in args.resume_from else args.resume_from
            if os.path.isfile(resume_path):
                sd = torch.load(resume_path, map_location="cpu")
                model.load_state_dict(sd, strict=True)
                print(f"[Fold {fold}] Loaded checkpoint: {resume_path}")
            else:
                print(f"[Fold {fold}] [WARN] resume_from not found: {resume_path} (training from scratch)")

        # freeze early layers if requested
        n_frozen = freeze_layers(model, args.freeze)
        if args.freeze != "none":
            print(f"[Fold {fold}] Freeze mode='{args.freeze}' | frozen params={n_frozen}")
        else:
            print(f"[Fold {fold}] Freeze mode='none'")
        ...
        best = -1.0; patience=args.patience; bad=0
    ...
    # overall
    pd.DataFrame(fold_rows).to_csv(os.path.join(args.outdir, "metrics_cnn.csv"), index=False)
